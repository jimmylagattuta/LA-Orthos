...
function ChatBoxWebMD(props) {
  const { csrfToken,setCsrfToken } = useCsrfToken();
  console.log('csrfToken ChatBoxWebMD', csrfToken);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/v1/send-email-webmd`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-Token': csrfToken,
